...
> SERVER_PUBLIC_URL=localhost, winget NSSM, thiếu TLS1.2, agent không trust Step-CA cho API HTTPS,
> tên service không gắn branch_id, v.v.
...
# HTTPS (khuyến nghị cho agent qua internet, cần HTTPS_ENABLED=true ở server + Step-CA):
SERVER_PUBLIC_URL=https://160.250.133.192:443 \
  OUTPUT_FILE=install.json node scripts/gen-client.js "Client Test"
# HOẶC HTTP (HQ LAN / VPN nội bộ):
# SERVER_PUBLIC_URL=http://160.250.133.192:3000 OUTPUT_FILE=install.json node scripts/gen-client.js "Client Test"
```
Lấy `root_ca.crt` từ Step-CA (xem `agent/CA_INSTALL.md`).
