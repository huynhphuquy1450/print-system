...
scp admin@160.250.133.192:/opt/print-service/CLIENT_SEND_JOB.ps1 C:\print-system\scripts\send-job.ps1
```
...
scp admin@160.250.133.192:/opt/print-service/scripts/send-job.sh ~/send-job.sh
chmod +x ~/send-job.sh
```
...
/opt/print-service/scripts/send-job.sh /tmp/test.pdf
```
...
```cmd
:: Setup 1 lần
scp admin@160.250.133.192:/opt/print-service/scripts/send-job.bat C:\print-system\scripts\send-job.bat
...
```bash
# 1. Login -> lấy JWT
JWT=$(curl -s -X POST http://160.250.133.192:3000/api/auth/login \
...
curl -X POST http://160.250.133.192:3000/api/print-jobs \
...
```powershell
$env:JWT = (Invoke-RestMethod -Uri "http://160.250.133.192:3000/api/auth/login" -Method POST -ContentType "application/json" -Body '{"client_id":"cli_3844de865fa7df32","client_secret":"<CLIENT_SECRET>"}').token
$env:B64 = [Convert]::ToBase64String([System.IO.File]::ReadAllBytes("C:\path\to\file.pdf"))
Invoke-RestMethod -Uri "http://160.250.133.192:3000/api/print-jobs" -Method POST -Headers @{Authorization="Bearer $env:JWT"} -ContentType "application/json" -Body "{`"branch_id`":`"br_001`",`"pdf_base64`":`"$env:B64`"}"
```
...
| `-a` / `-ApiBase` | `http://160.250.133.192:3000` | URL server (đổi khi test local) |
...
```
[Your machine]                  [VPS 160.250.133.192]              [Agent br_001 machine]
   |                                    |                                    |
...
sqlite3 /opt/print-service/data/jobs.db "SELECT id, status, printed_at FROM jobs ORDER BY created_at DESC LIMIT 1"
...
| `[FATAL] Login fail` | Sai client_id/secret, hoặc VPS offline | `Test-NetConnection 160.250.133.192 -Port 3000` (Windows) / `nc -zv 160.250.133.192 3000` (Linux) |
| `Send fail: HTTP 404` | Branch không tồn tại | Check `curl http://160.250.133.192:3000/api/branches -H "Authorization: Bearer $JWT"` |
