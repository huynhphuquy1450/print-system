...
router.get('/', verifyClient, (req, res) => {
  const branches = stmts.listBranches.all().map((b) => ({
    id: b.id,
    name: b.name,
    location: b.location,
    status: b.status,
...
router.get('/:id', verifyClient, (req, res) => {
  const branch = stmts.getBranchById.get(req.params.id);
 ...
 */
router.post('/:id/regen-token', verifyClient, (req, res) => {
  const branch = stmts.getBranchById.get(req.params.id);
  if (!branch) return res.status(404).json({ error: 'Branch not found' });

  const newToken = generateAgentToken();
  stmts.updateBranchToken.run({
