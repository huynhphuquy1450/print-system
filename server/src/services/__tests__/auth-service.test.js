...
describe('auth-service', () => {
 ...
 test('returns null if client not found in DB', async () => {
 stmts.getClientById.get.mockResolvedValue(null);
 await expect(verifyClientCredentials('missing', 'pass')).resolves.toBeNull();
 });

 test('returns null if client is_active=0', async () => {
 stmts.getClientById.get.mockResolvedValue({
 id: TEST_CLIENT_ID,
 name: TEST_CLIENT_NAME,
 secret_hash: BCRYPT_HASH,
 is_active: 0,
 });
 await expect(verifyClientCredentials(TEST_CLIENT_ID, 'any')).resolves.toBeNull();
 });

 test('returns null if bcrypt compare fails (wrong secret)', async () => {
 stmts.getClientById.get.mockResolvedValue({
 id: TEST_CLIENT_ID,
 name: TEST_CLIENT_NAME,
 secret_hash: BCRYPT_HASH,
 ...
 await expect(verifyClientCredentials(TEST_CLIENT_ID, 'wrong')).resolves.toBeNull();
 });
 });
