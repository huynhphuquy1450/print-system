...
const BRANCH = 'br_001';
// Base64 của "%PDF-1.4\nfake content" — đủ lớn và có magic bytes hợp lệ
...
describe('job-service', () => {
  ...
  describe('createJob', () => {
    test('happy path: returns job_id, writes PDF, inserts DB, publishes MQTT, marks sent', async () => {
      ...
      expect(pubPayload.pdf_base64).toBe(VALID_PDF_B64);
    ...
    test('throws 400 if branchId missing', async () => {
      await expect(
        createJob({ branchId: '', pdfBase64: VALID_PDF_B64, metadata: {}, clientId: 'c' })
      ).rejects.toMatchObject({ status: 400, message: /branch_id is required/ });
      expect(fs.writeFileSync).not.toHaveBeenCalled();
      expect(stmts.insertJob.run).not.toHaveBeenCalled();
    });

    test('throws 400 if pdfBase64 missing', async () => {
      await expect(
        createJob({ branchId: BRANCH, pdfBase64: '', metadata: {}, clientId: 'c' })
      ).rejects.toMatchObject({ status: 400, message: /pdf_base64 is required/ });
    });

    test('throws 404 if branch not found', async () => {
      stmts.getBranchById.get.mockReturnValue(null);
      await expect(
        createJob({ branchId: 'br_missing', pdfBase64: VALID_PDF_B64, metadata: {}, clientId: 'c' })
    ...
    test('throws 400 if PDF missing %PDF- magic bytes', async () => {
      const badB64 = Buffer.from('not a pdf at all').toString('base64');
      await expect(
        createJob({ branchId: BRANCH, pdfBase64: badB64, metadata: {}, clientId: 'c' })
    ...
    test('printer undefined → inserted as null', async () => {
      await createJob({ branchId: BRANCH, pdfBase64: VALID_PDF_B64, metadata: {}, clientId: 'c' });
      expect(stmts.insertJob.run.mock.calls[0][0].printer).toBeNull();
    });

    test('metadata undefined → stored as "{}"', async () => {
      await createJob({ branchId: BRANCH, printer: 'p', pdfBase64: VALID_PDF_B64, clientId: 'c' });
      expect(stmts.insertJob.run.mock.calls[0][0].metadata).toBe('{}');
    });

    test('clientId undefined → stored as null', async () => {
      await createJob({ branchId: BRANCH, pdfBase64: VALID_PDF_B64, metadata: {} });
